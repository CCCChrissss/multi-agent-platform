...
兩個 workflow 共用同一組 `stt`/`check`/`notified` agent server（同一批 port），跑哪一個是**啟動時**的選擇，不是每次請求各自決定——見下方「切換示範 workflow」。
...
編排層      Master Agent --event_bus--> Worker x3           LangGraph StateGraph
            master_agent.py             worker.py
                     ...
                     v                                             |
Agent 層    agents/{stt,check,notified}/server.py                  |
                     |                                             |
                     +-------> llm/stt_agent.py <-----------------+
...
基礎建設層           MCPGateway（RBAC）          LiteLLM Gateway
                     mcp_servers/gateway.py      gateway/
                                     |                    |
MCP server 層  mcp_servers/{stt,format_check,lookup,notified,memory}/     |
                                     |                             |
...
打開 `.env` 填 `GEMINI_API_KEY`（兩個示範場景都會用到：`notified` 走 `gemini-cheap`、除外責任場景的 `check` 走 `gemini-strong`）。`PERSISTENCE_DATABASE_URL` 預設值對應上一步建的 DB，本機 Postgres 有設帳密才要改。
...
| `stt-agent` | 8003 | stt agent 的 HTTP service |
| `check-agent` | 8004 | check agent 的 HTTP service |
| `notified-agent` | 8005 | notified agent 的 HTTP service |

這個指令會佔用這個 terminal、把 7 個服務的 log 用不同顏色 prefix 混在一起印出來；`Ctrl+C` 一次就會全部連帶關掉，不會留下殘留 process。

**確認有起來**（另開一個 terminal）：

```bash
# 7 個 port 都要是 LISTENING
for p in 11434 4000 8001 8002 8003 8004 8005; do
  printf "%s: " $p; lsof -ti :$p >/dev/null 2>&1 && echo OK || echo FAILED
done

# LiteLLM Gateway 讀到 gateway/config.yaml 的 5 個模型
curl -s http://localhost:4000/v1/models | python3 -m json.tool | grep '"id"'
```

第二個指令要印出 `local-qwen`、`gemini-cheap`、`gemini-strong`、`breeze-asr`、`local-embed`。任何一項對不上，見 [docs/setup.md](docs/setup.md)。
...
> - 同步模式其實用不到 8003~8005 三個 agent service，但一起起來也無妨。
...
**B-1. 啟動 Master Agent 與三個 worker**（另開一個 terminal）
...
| `worker-stt` / `worker-check` / `worker-notified` | 各自認領自己那一步的命令，執行完發出完成事件 |
...
兩個 honcho terminal 各按一次 `Ctrl+C`，各自的 process 都會連帶關掉（trigger 是一次性執行，跑完自動結束）。如果不小心留下殘留 process：
...
Procfile.workers         honcho 用，一個指令啟動事件驅動模式的 Master Agent + 三個 worker
...
agents/                  三個 agent 的獨立 HTTP service + client + 共用的 request/response envelope
...
- [docs/setup.md](docs/setup.md) — 安裝疑難排解（pgvector 編譯、Postgres 連線、Ollama 撞 port、LiteLLM 起不來）
- [CLAUDE.md](CLAUDE.md) — 平台目標，以及「什麼算平台能力、什麼算場景邏輯」的判準
- [docs/harness-engineering-principles.md](docs/harness-engineering-principles.md) — 新增/調整 agent、tool 前要先看的原則與檢查清單
- [docs/agent-api-contract.md](docs/agent-api-contract.md) — agent 的請求/回應 envelope，以及它為什麼跟傳輸層脫鉤
- [docs/event-driven-multi-agent-coordination-plan.md](docs/event-driven-multi-agent-coordination-plan.md) — 事件驅動模式的完整設計
- [docs/mcp-gateway-rbac-plan.md](docs/mcp-gateway-rbac-plan.md) — 「哪個 agent 能用哪些 tool」的治理設計
- [docs/long-term-memory-plan.md](docs/long-term-memory-plan.md) — 長期記憶（跨執行的知識）導入計畫
- [docs/knowledge-distillation-plan.md](docs/knowledge-distillation-plan.md) — episodic -> procedural 規則蒸餾與人工審核的品質關卡（M5）
- [docs/exclusion-scenario-plan.md](docs/exclusion-scenario-plan.md) — 除外責任場景的設計：`browse()` 漸進式揭露、保單條款怎麼分層存、`check`/`notified` 怎麼改
- [docs/observability.md](docs/observability.md) — 怎麼查稽核歷史/執行狀態、Postgres 資料表一覽
- [docs/testing.md](docs/testing.md) — smoke test 指令、記憶蒸餾 pipeline（P0-P5）手動試跑步驟
