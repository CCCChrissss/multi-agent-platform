...
若 PowerShell execution policy 阻擋本機腳本，可以對本次程序執行：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\dev.ps1 check
```

此形式不修改全機或使用者的持久 execution policy。

## 3. 準備自己的資料庫

在 VS Code 終端機確認 `psql --version`。若找不到，將自己 PostgreSQL 的 `bin` 加入 PATH。
以下資料庫名称只是共用範例；不同開發者可以自行更改，並同步寫入自己的 `.env`。

```powershell
psql -h localhost -U postgres -d postgres -c "CREATE DATABASE agent_architecture_test;"
psql -h localhost -U postgres -d agent_architecture_test -c "SELECT name, default_version FROM pg_available_extensions WHERE name='vector';"
```

`psql` 會提示輸入自己的 PostgreSQL 密碼。若資料庫已存在，確認用途後略過建立，不要刪除它。
若 extension 已可用：

```powershell
psql -h localhost -U postgres -d agent_architecture_test -c "CREATE EXTENSION IF NOT EXISTS vector;"
psql -h localhost -U postgres -d agent_architecture_test -c "SELECT extversion FROM pg_extension WHERE extname='vector';"
```

成功條件為最後查詢有一列版本。若沒有 vector，依 [pgvector 官方 Windows 編譯說明](https://github.com/pgvector/pgvector#installation-notes---windows) 安裝 Visual Studio Build Tools 的 C++ 工具。
安裝系統工具可使用 installer；編譯命令仍可在 VS Code 的 PowerShell terminal 執行。
只有這個寫入 PostgreSQL 安裝目錄的步驟需要用管理員身分開啟 VS Code。
...
$env:PGROOT = Split-Path -Parent (Split-Path -Parent (Get-Command psql -ErrorAction Stop).Source)
if (-not (Test-Path (Join-Path $env:PGROOT 'include\server\postgres.h'))) {
    throw 'psql 的位置不是完整 PostgreSQL 安裝，請確認 PATH 與 PGROOT。'
}
...
安裝後回到一般權限的 VS Code，執行前面的 CREATE EXTENSION 與版本查詢。
本次未實際重新安裝或編譯；不同 Build Tools/PostgreSQL 組合仍需驗證。
...
用 VS Code 開啟 `.env`，填寫自己的 `PERSISTENCE_DATABASE_URL`。
URL 內帳號／密碼如含保留字元，需使用 percent encoding；不要提交或貼出實際值。
設定責任：
...
- 沒有 server：在獨立 VS Code terminal 執行 `.\scripts\dev.ps1 ollama`，保持終端機開啟；這份才由專案管理。
...
```powershell
# Terminal B
.\scripts\dev.ps1 workers -Workflow stt_exclusion_notify
# Terminal C
.\scripts\dev.ps1 trigger -Workflow stt_exclusion_notify
```
